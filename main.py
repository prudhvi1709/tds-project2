...
CODE_GENERATION_PROMPT = "Generate only the exact code needed to solve the problem. If the question contains any formula's, analyze it and then come up with a clear and concise answer. No explanations, comments, or additional text. Return ONLY the executable code."
...
def find_similar_question(query, questions_data):
    # Add special handling for GitHub configuration questions
    if "github_user" in query.lower() or "repo_name" in query.lower():
        return {
            "question": "GitHub Configuration",
            "code": "yes",
            "type": "github_config"
        }, 1.0
    
    # Add special handling for formula questions
    if "=SUM" in query or "=ARRAY" in query or "formula" in query.lower():
        if "google sheets" in query.lower() or "ARRAY_CONSTRAIN" in query.lower():
            return {
                "question": "Google Sheets Formula",
                "code": "yes",
                "type": "sheets_formula"
            }, 1.0
...
def generate_code_for_question(question: str, code_type: str, file_context: str, temp_dir: str, processed_files: List[str] = None) -> Dict[str, Any]:
    ...
    # Add special handling for GitHub configuration
    if code_type == "github_config":
        # Extract GitHub username and repo name from the question
        github_user = None
        repo_name = None
        
        if "github_user" in question:
            user_match = re.search(r'github_user\s*=\s*(\w+)', question)
            if user_match:
                github_user = user_match.group(1)
        
        if "repo_name" in question:
            repo_match = re.search(r'repo_name\s*:\s*(\w+)', question)
            if repo_match:
                repo_name = repo_match.group(1)
        
        response = "GitHub Configuration:\n"
        if github_user:
            response += f"GitHub Username: {github_user}\n"
        if repo_name:
            response += f"Repository Name: {repo_name}"
    ...
    # Handle formula questions
    if code_type in ["sheets_formula", "excel_formula"]:
        # Extract the formula from the question using regex
        formula_match = re.search(r'=(.*?)(?=What is the result?|$)', question)
        if formula_match:
            formula = "=" + formula_match.group(1).strip()
        else:
            # Fallback to looking for = if regex doesn't find a match
            for line in question.split("\n"):
                if "=" in line:
                    formula = line[line.find("="):].strip()
                    break
            else:
                formula = ""
        ...
        if not formula:
            return {
                "success": False,
                "error": "No formula found in the question"
            }
        
        if code_type == "sheets_formula":
            return {
                "success": True,
                "result": "To get the result, please enter this formula in Google Sheets: " + formula
            }
        else:  # excel_formula
            return {
                "success": True,
                "result": "To get the result, please enter this formula in Excel 365: " + formula
            }
